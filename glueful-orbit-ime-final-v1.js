...
(function(){
  ...
  function install(){
    ...
    s.textContent=`
      ...
      #${ROOT}.open .orbit5-head{flex:0 0 64px!important;height:64px!important;min-height:64px!important;position:relative!important;z-index:100!important;transform:none!important}
  ...
  function sync(){
    ...
    r.style.setProperty('--orbit-visible-height',h+'px');
    r.style.setProperty('top',top+'px','important');
    r.style.setProperty('bottom','auto','important');
    r.style.setProperty('height',h+'px','important');
  }
  function lockPage(){
    const r=document.getElementById(ROOT);if(!r?.classList.contains('open'))return;
    ...
    setTimeout(sync,180);setTimeout(sync,420);
  }
  function boot(){
    install();
    ...
    new MutationObserver(()=>{const r=document.getElementById(ROOT);if(r?.classList.contains('open'))sync()}).observe(document.documentElement,{attributes:true,attributeFilter:['class','style'],subtree:true});
  }
