...
(function () {
  ...
  async function waitForLibrary(name, timeout = DOCX_PREVIEW_TIMEOUT) {
    ...
    while (!window[name]) {
      if (Date.now() - started >= timeout) throw new Error(`${name} importer did not finish loading.`);
  ...
  async function convertPdfWithAdobe(pdfBuffer) {
    ...
    if (bytes[0] !== 0x25 || bytes[1] !== 0x50 || bytes[2] !== 0x44 || bytes[3] !== 0x46) throw new Error('The uploaded file does not appear to be a PDF.');
    const client = window.supabaseClient;
    const config = getSupabaseConfig();
    if (!client?.auth || !config.url || !config.key) throw new Error('Supabase authentication client is not available for PDF conversion.');
    const { data: sessionData, error: sessionError } = await client.auth.getSession();
    if (sessionError || !sessionData?.session?.access_token) throw new Error('Your Glueful session is unavailable. Please sign in again.');
    ...
    if (!contentType.includes('application/vnd.openxmlformats-officedocument.wordprocessingml.document')) throw new Error('Adobe conversion returned an unexpected content type.');
  ...
  async function renderDocxWithLayout(docxBuffer, ed) {
    ...
    renderHost.className = 'glueful-docx-render-host';
    renderHost.contentEditable = 'true';
    renderHost.spellcheck = true;
    ed.replaceChildren(renderHost, styleHost);
    ed.contentEditable = 'true';
    ed.classList.remove('pdf-structured-canvas', 'pdf-native-canvas');
    ed.classList.add('glueful-word-document-mode', 'glueful-docx-layout-mode', 'job-resume-master-imported');
  ...
  async function docxToMammothHtml(docxBuffer) {
    ...
    if (Array.isArray(result?.messages) && result.messages.length) console.info('[Glueful Resume Studio] Mammoth fallback messages:', result.messages);
  ...
  async function sourceFileToEditor(file, ed) {
    ...
    if (name.endsWith('.docx')) {
      ...
      try { return await renderDocxWithLayout(docxBuffer, ed); }
      catch (layoutError) { console.warn('[Glueful Resume Studio] DOCX layout renderer failed; using Mammoth fallback:', layoutError); const html = await docxToMammothHtml(docxBuffer); ed.innerHTML = html; return { html, imported: true, pdfConverted: false, renderer: 'mammoth-fallback' }; }
    }
    if (name.endsWith('.pdf')) {
      ...
      try { return await renderDocxWithLayout(docxBuffer, ed); }
      catch (layoutError) { console.warn('[Glueful Resume Studio] DOCX layout renderer failed; using Mammoth fallback:', layoutError); const html = await docxToMammothHtml(docxBuffer); ed.innerHTML = html; return { html, imported: true, pdfConverted: true, renderer: 'mammoth-fallback' }; }
    }
    return null;
  }
  function preparePlainEditor(ed) {
    if (!ed) return;
    ed.classList.remove('pdf-structured-canvas', 'pdf-native-canvas', 'glueful-docx-layout-mode');
  ...
  async function loadMasterIntoEditor() {
    ...
    if (imported?.html) {
      if (imported.renderer !== 'docx-preview') preparePlainEditor(ed);
      showConversionNote(imported.pdfConverted ? 'PDF converted to an editable Word document while preserving Word page layout. Your master resume remains protected.' : 'Editable master resume loaded. Your master resume remains protected.');
      return imported;
    }
    if (typeof window.resumeTextToEditorHtml === 'function') ed.innerHTML = window.resumeTextToEditorHtml(base);
    else ed.innerHTML = base.split(/\r?\n/).map((line) => line.trim() ? `<p>${line.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</p>` : '<p><br></p>').join('');
    preparePlainEditor(ed);
    showConversionNote(master ? 'Loaded from your Candidate Profile master resume. This job-specific copy is temporary.' : 'No readable master resume is available yet. Upload one in Candidate Profile.');
  ...
  async function openJobResumeEditor(id) {
    ...
    finally { clearConversionUi(); }
    if (typeof window.updateJobResumeEditorAts === 'function') { try { window.updateJobResumeEditorAts(); } catch (_) {} }
    if (typeof window.gluefulResumeStudioEnhance === 'function') { try { window.gluefulResumeStudioEnhance(); } catch (_) {} }
  }
  async function resetJobResumeToMaster() {
    const ed = editor();
    if (!ed) return;
    try { clearConversionUi(); await loadMasterIntoEditor(); if (typeof window.updateJobResumeEditorAts === 'function') { try { window.updateJobResumeEditorAts(); } catch (_) {} } ed.focus(); }
    catch (error) { console.error('[Glueful Resume Studio Adobe] reset failed:', error); if (typeof window.showError === 'function') window.showError(error?.message || 'Could not reset the temporary resume.'); }
    finally { clearConversionUi(); }
  }
  window.gluefulAdobeResumeStudio = { version: '2.3.0', functionName: FUNCTION_NAME, pipeline: ['PDF master', 'Supabase glueful-pdf-to-docx', 'Adobe PDF Services', 'real DOCX', 'docx-preview layout renderer', 'existing contenteditable Word-style editor'], openJobResumeEditor, resetJobResumeToMaster };
