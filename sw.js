const CACHE_NAME="glueful-cache-v55-jobs-v11-stable";
...
const JOBS_V11="./glueful-jobs-discover-v11-stable.js";
const RUNTIME=[RENDER_DIAGNOSTICS_SCRIPT,FIXED_PDF_BOOTSTRAP,FIXED_PDF_MODEL,FIXED_PDF_IMPORTER,FIXED_PDF_RENDERER,FIXED_PDF_UX,FIXED_PDF_CONTROLLER,FIXED_PDF_DOCX_EXPORT_V2,FIXED_PDF_TYPOGRAPHY,RESUME_IMPORT_GUARD,PDF_EXPORT_FIX,JOBS_AUTH,JOBS_V11];
...
async function buildAuthoritativeIndex(request,preloadResponse){const response=await networkResponse(request,preloadResponse);if(!response?.ok)return response;const type=response.headers.get("content-type")||"";if(!type.includes("text/html"))return response;let html=await response.text();html=stripCompetingRuntime(html);const scripts=[`<script src="${RENDER_DIAGNOSTICS_SCRIPT}?v=20260820-13" data-glueful-runtime="render-diagnostics"></script>`,`<script src="${FIXED_PDF_BOOTSTRAP}?v=20260820-fixedpdf28" data-glueful-runtime="fixed-pdf-bootstrap"></script>`,`<script src="${JOBS_V11}?v=20260820-jobs-v11-stable" data-glueful-runtime="jobs-discover-v11"></script>`];const marker="</body>",block=scripts.join("\n"),injected=html.includes(marker)?html.replace(marker,`${block}\n${marker}`):`${html}\n${block}`;const headers=new Headers(response.headers);headers.set("Content-Type","text/html; charset=UTF-8");headers.set("Cache-Control","no-store, no-cache, must-revalidate");return new Response(injected,{status:response.status,statusText:response.statusText,headers})}
