...
(function(){
  ...
  (function installPrepaintGate(){
    ...
    style.textContent='html.glueful-booting body{visibility:hidden!important;}html.glueful-booting #glueful-splash{display:none!important;}';
    (document.head||root).appendChild(style);
    window.__GLUEFUL_RELEASE_PAINT__=function(){if(root)root.classList.remove('glueful-booting');};
  })();
  ...
  function waitForFinalRuntime(timeout){return new Promise(function(resolve){var done=false,start=Date.now();function finish(){if(done)return;done=true;resolve();}window.addEventListener('glueful-dashboard-ready',finish,{once:true});(function check(){if(done)return;if(window.__GLUEFUL_DASHBOARD_READY__)return finish();if(Date.now()-start>timeout)return finish();setTimeout(check,25);})();});}
  async function boot(){
    try{
      ...
      try{await load('./glueful-desktop-tablet-sidebar-persist-v2.js?v=4');}catch(error){console.warn('[Glueful] Persistent sidebar layer unavailable:',error);}
      try{await load('./glueful-feature-loader-v1.js?v=172');}catch(error){console.warn('[Glueful] Direct feature-loader bootstrap failed:',error);}
    ...
    finally{if(typeof window.__GLUEFUL_RELEASE_PAINT__==='function')window.__GLUEFUL_RELEASE_PAINT__();}
  }
  void boot();
})();
