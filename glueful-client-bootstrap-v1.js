...
(function(){
  ...
  (function installResumeShellAuthority(){try{
    function start(){
      ...
      function paint(){
        ...
        v.style.setProperty('position','relative','important');v.style.setProperty('left','auto','important');v.style.setProperty('right','auto','important');v.style.setProperty('transform','none','important');v.style.setProperty('margin-left','260px','important');v.style.setProperty('margin-right','0','important');v.style.setProperty('width','calc(100vw - 260px)','important');v.style.setProperty('max-width','none','important');v.style.setProperty('min-height','100vh','important');v.style.setProperty('box-sizing','border-box','important');v.style.setProperty('padding','0','important');v.style.setProperty('background','#f7f8fb','important');v.style.setProperty('color','#111827','important');
      }
      paint();[100,300,700,1200,2000,3500,5000,8000,12000].forEach(function(t){setTimeout(paint,t)});
    }
    if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
  }catch(error){console.warn('[Glueful] Resume shell authority unavailable:',error);}})();
  ...
  async function boot(){try{
    ...
    try{await load('./glueful-feature-loader-v1.js?v=196');}catch(error){console.warn('[Glueful] Feature loader unavailable:',error);}
