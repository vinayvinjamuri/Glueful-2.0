...
(function(){
  ...
  function load(src){
    ...
    const p=new Promise((resolve,reject)=>{
      ...
      if(old){loaded.add(src);resolve();return;}
  ...
  function loadGroup(name){
    ...
    return GROUPS[name].reduce((p,src)=>p.then(()=>load(src)),Promise.resolve())
      .catch(error=>{
        delete window.__GLUEFUL_LAZY_GROUPS__[name];
        console.error('[Glueful] Lazy runtime failed:',name,error);
      });
  }

  function hasJobs(){return !!document.getElementById('jobs-view')||!!document.getElementById('glueful-jobs-v15');}
  function hasResume(){return !!document.getElementById('job-resume-editor-modal')||!!document.querySelector('.glueful-fixed-pages');}
  function hasGmail(){return !!document.querySelector('[data-view="gmail"],#gmail-view,.gmail-view');}

  function bootExisting(){
    if(hasJobs()) void loadGroup('jobs');
    if(hasResume()) void loadGroup('resume');
    if(hasGmail()) void loadGroup('gmail');
  }
  ...
  window.gluefulLazyRuntime={loadGroup,version:'1.0.0'};
})();
