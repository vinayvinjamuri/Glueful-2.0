...
(function(){
  ...
  function score(j){
    ...
    const p=profileText();
    if(p){
      const pt=new Set(p.split(/[^a-z0-9+#.]+/).filter(w=>w.length>=3));
      const jt=new Set(text(j).split(/[^a-z0-9+#.]+/).filter(w=>w.length>=3));
      let overlap=0;jt.forEach(w=>{if(pt.has(w))overlap++});
      s+=Math.min(45,overlap*3);
    }
  ...
  async function loadInventory(){
    ...
    try{
      ...
      const seen=new Set();
      inventory=rows.filter(j=>{
        const key=String(j.id||j.job_id||`${title(j)}|${company(j)}|${location(j)}|${j.application_url||j.job_url||''}`);
        if(seen.has(key))return false;seen.add(key);return true;
      });
      window.gluefulJobsMarketplace={getJobs:()=>inventory.slice(),getCompanies:()=>companyRows(),refresh:loadInventory};
      return inventory;
    }catch(e){console.warn('[Glueful Marketplace] inventory load failed',e);return []}
  }

  function companyRows(){
    const map=new Map();inventory.forEach(j=>{const c=company(j);map.set(c,(map.get(c)||0)+1)});
    return [...map].sort((a,b)=>b[1]-a[1]);
  }

  function sheet(titleText,items,mode){
    const old=document.querySelector('.gmkt-layer');if(old)old.remove();
    const layer=document.createElement('div');layer.className='gmkt-layer';
    const isCompanies=mode==='companies';
    const body=isCompanies?companyRows().map(([name,count])=>`<button class="gmkt-company" data-company="${esc(name)}" type="button"><div class="gmkt-logo">${esc(initials(name))}</div><div><strong>${esc(name)}</strong><span>${count} ${count===1?'open role':'open roles'}</span></div><b>›</b></button>`).join(''):items.map(j=>`<button class="gmkt-row" data-job="${esc(j.id||j.job_id)}" type="button"><div class="gmkt-logo">${logo(j)?`<img src="${esc(logo(j))}" alt="">`:esc(initials(company(j)))}</div><div><strong>${esc(title(j))}</strong><span>${esc(company(j))}${location(j)?' · '+esc(location(j)):''}</span><small>${score(j)}% match${j.posted_at?' · '+esc(new Date(j.posted_at).toLocaleDateString()):''}</small></div></button>`).join('');
    layer.innerHTML=`<div class="gmkt-panel"><header><div><h2>${esc(titleText)}</h2><p>${isCompanies?companyRows().length+' companies':items.length+' jobs'} · loaded from the full active inventory</p></div><button data-close type="button">×</button></header><div class="gmkt-search"><input id="gmkt-q" type="search" placeholder="Search ${isCompanies?'companies':'jobs, skills or roles'}…"></div><div class="gmkt-body">${body||'<div class="gmkt-empty">No results found.</div>'}</div></div>`;
    document.body.appendChild(layer);document.body.style.overflow='hidden';
    const bodyEl=layer.querySelector('.gmkt-body');
    layer.addEventListener('click',e=>{
      ...
      if(jobBtn){const j=inventory.find(x=>String(x.id||x.job_id)===String(jobBtn.dataset.job));if(j&&typeof window.openJobDetails==='function'){layer.remove();document.body.style.removeProperty('overflow');window.openJobDetails(j.id||j.job_id)}return}
      const companyBtn=e.target.closest('[data-company]');
      if(companyBtn){const name=companyBtn.dataset.company;const jobs=inventory.filter(j=>company(j)===name).sort((a,b)=>score(b)-score(a));sheet(name,jobs,'jobs')}
    });
    layer.querySelector('#gmkt-q').addEventListener('input',e=>{
      const q=lower(e.target.value);
      if(isCompanies){
        const rows=companyRows().filter(([n])=>lower(n).includes(q));
        bodyEl.innerHTML=rows.map(([name,count])=>`<button class="gmkt-company" data-company="${esc(name)}" type="button"><div class="gmkt-logo">${esc(initials(name))}</div><div><strong>${esc(name)}</strong><span>${count} ${count===1?'open role':'open roles'}</span></div><b>›</b></button>`).join('')||'<div class="gmkt-empty">No companies found.</div>';
      }else{
        const rows=inventory.filter(j=>!q||text(j).includes(q)).sort((a,b)=>score(b)-score(a));
        bodyEl.innerHTML=rows.map(j=>`<button class="gmkt-row" data-job="${esc(j.id||j.job_id)}" type="button"><div class="gmkt-logo">${logo(j)?`<img src="${esc(logo(j))}" alt="">`:esc(initials(company(j)))}</div><div><strong>${esc(title(j))}</strong><span>${esc(company(j))}${location(j)?' · '+esc(location(j)):''}</span><small>${score(j)}% match</small></div></button>`).join('')||'<div class="gmkt-empty">No matching jobs.</div>';
      }
    });
  }

  function inject(){
    const root=document.querySelector('#glueful-jobs-v13,#glueful-jobs-v14,#glueful-jobs-v15');
    if(!root||root.querySelector('.gmkt-marketplace'))return;
    const section=document.createElement('section');section.className='gmkt-marketplace';
    section.innerHTML=`<div class="gmkt-market-head"><div><h2>Explore the full market</h2><p>Browse beyond your curated feed</p></div></div><div class="gmkt-actions"><button data-all-jobs type="button"><strong>All jobs</strong><span>Browse the full job inventory</span></button><button data-all-companies type="button"><strong>All companies</strong><span>Explore every employer with open roles</span></button></div>`;
    root.querySelector('.g13-page,.g14-page')?.appendChild(section);
...
`;document.head.appendChild(s)}
  ...
  async function boot(){
    ...
    await loadInventory();
    let tries=0;
    const timer=setInterval(()=>{inject();if(document.querySelector('#glueful-jobs-v13,#glueful-jobs-v14,#glueful-jobs-v15')){clearInterval(timer)}else if(++tries>30)clearInterval(timer)},300);
  }
