...
(function(){
'use strict';
const VERSION='20260820-fixedpdf20';
...
function enforceAuthority(){if(!window.__gluefulFixedPdfScheduled)return;const studio=window.gluefulFixedPdfResumeStudio;if(!studio||typeof studio.open!=='function')return;if(window.openJobResumeEditor!==studio.open)window.openJobResumeEditor=studio.open;if(typeof studio.reset==='function'&&window.resetJobResumeToMaster!==studio.reset)window.resetJobResumeToMaster=studio.reset;window.__gluefulFixedPdfAuthorityActive=true;window.__GLUEFUL_RENDER_DEBUG__=Object.assign(window.__GLUEFUL_RENDER_DEBUG__||{},{renderer:'fixed-pdf',fixedLoaded:true,fixedReady:!!window.__gluefulFixedPdfReady,fixedScheduled:true,fixedAuthorityActive:true,adobeLoaded:!!window.gluefulAdobeResumeStudio,serviceWorkerControlled:!!navigator.serviceWorker?.controller,openJobResumeEditorSource:window.openJobResumeEditor?.name||'fixed-controller',controllerVersion:window.gluefulFixedPdfResumeStudio?.getState?.()?.renderer||null});}
...
async function boot(){installAuthorityGate();startAuthorityWatchdog();try{for(const [src,id] of ASSETS)await load(src,id);if(!window.pdfjsLib)throw new Error('PDF.js runtime did not initialize.');window.__gluefulFixedPdfReady=true;window.__GLUEFUL_RENDER_DEBUG__=Object.assign(window.__GLUEFUL_RENDER_DEBUG__||{},{renderer:'fixed-pdf',fixedLoaded:true,fixedReady:true,fixedScheduled:true,serviceWorkerControlled:!!navigator.serviceWorker?.controller});window.gluefulFixedPdfResumeStudio?.activate?.();enforceAuthority();console.info('[GLUEFUL] FIXED PDF RENDERER ACTIVE',VERSION)}catch(error){window.__GLUEFUL_RENDER_DEBUG__=Object.assign(window.__GLUEFUL_RENDER_DEBUG__||{},{renderer:'fixed-pdf',fixedLoaded:true,fixedReady:false,fixedScheduled:true,error:String(error?.message||error)});console.error('[Glueful Resume Studio] fixed-PDF bootstrap failed:',error)}}
