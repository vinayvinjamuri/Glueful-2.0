...
(function(){
'use strict';
const VERSION='20260820-fixedpdf21';
  ...
  return new Promise((resolve,reject)=>{
    ...
    const timer=setInterval(()=>{
      ...
      if(typeof fn==='function'){
        clearInterval(timer);resolve(fn);return;
      }
...
function installAuthorityGate(){
  ...
  window.openJobResumeEditor=async function(id){
    try{return await(await waitForRuntime('open'))(id)}
    catch(error){
  ...
  window.resetJobResumeToMaster=async function(id){
    try{return await(await waitForRuntime('reset'))(id)}
    catch(error){
...
function enforceAuthority(){
  ...
  window.__GLUEFUL_RENDER_DEBUG__=Object.assign(window.__GLUEFUL_RENDER_DEBUG__||{}, {
    renderer:'fixed-pdf',
    fixedLoaded:true,
    fixedReady:!!window.__gluefulFixedPdfReady,
    fixedScheduled:true,
...
function getRememberedResumeEditorJob(){
  try{return sessionStorage.getItem('glueful.activeResumeEditorJobId')||''}catch(_){return ''}
}
function fixedSurfaceIsAttached(){
  const surface=window.gluefulResumeFixedPageState?.surface;
  return !!surface && document.contains(surface) && !!document.querySelector('.glueful-fixed-pages');
}
function repairDetachedResumeEditor(){
  if(repairInFlight||Date.now()<repairCooldownUntil||repairAttempts>=3)return;
  const modal=document.getElementById('job-resume-editor-modal');
  if(!modal?.classList.contains('open'))return;
  const id=getRememberedResumeEditorJob();
  const studio=window.gluefulFixedPdfResumeStudio;
  if(!id||!studio||typeof studio.open!=='function'||fixedSurfaceIsAttached())return;
  if(typeof window.findActiveJobById!=='function')return;
  const job=window.findActiveJobById(id);
  if(!job)return;
  repairAttempts+=1;
  repairCooldownUntil=Date.now()+2000;
  console.info('[Glueful Resume Studio] repairing detached fixed-PDF surface after startup:',id);
  repairInFlight=Promise.resolve(studio.open(id)).catch(error=>{
    console.warn('[Glueful Resume Studio] detached fixed-PDF repair deferred:',error);
  }).finally(()=>{repairInFlight=null});
}
function startAuthorityWatchdog(){
  if(authorityWatchdog)return;
  enforceAuthority();
  authorityWatchdog=setInterval(()=>{
    enforceAuthority();
    repairDetachedResumeEditor();
  },100);
}
...
async function boot(){
  ...
  try{
    ...
    window.__GLUEFUL_RENDER_DEBUG__=Object.assign(window.__GLUEFUL_RENDER_DEBUG__||{}, {
      renderer:'fixed-pdf',fixedLoaded:true,fixedReady:true,fixedScheduled:true,
      serviceWorkerControlled:!!navigator.serviceWorker?.controller,
      bootstrapVersion:VERSION
    });
    ...
    window.__GLUEFUL_RENDER_DEBUG__=Object.assign(window.__GLUEFUL_RENDER_DEBUG__||{}, {
      ...
      error:String(error?.message||error),bootstrapVersion:VERSION
    });
