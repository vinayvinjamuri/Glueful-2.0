...
function load(src,id){return new Promise((resolve,reject)=>{const existing=document.getElementById(id);if(existing){if(existing.dataset.loaded==='true')return resolve();existing.addEventListener('load',resolve,{once:true});existing.addEventListener('error',()=>reject(new Error(`Failed to load ${src}`)),{once:true});return}const s=document.createElement('script');s.id=id;s.src=src+'?v='+VERSION;s.async=false;s.onload=()=>{s.dataset.loaded='true';resolve()};s.onerror=()=>reject(new Error(`Failed to load ${src}`));document.body.appendChild(s)})}
async function boot(){for(const [src,id] of ASSETS)await load(src,id);window.gluefulFixedPdfResumeStudio?.activate?.();console.info('[Glueful Resume Studio] fixed-PDF controller bootstrap loaded; PDF masters use fixed-page renderer',VERSION)}
