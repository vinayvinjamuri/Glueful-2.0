/* Glueful Gmail integration v9: multi-account UI with dashboard-scoped sync control. */
(function () {
  "use strict";
  ...
  function ensureDashboardButton() {
    ensureStyles();
    const existing = document.getElementById("glueful-dashboard-gmail-sync");
    if (existing) { updateDashboardButton(); return; }
    const host = dashboardHost();
    if (!host) { setTimeout(ensureDashboardButton,1200); return; }
    const b=document.createElement("button"); b.id="glueful-dashboard-gmail-sync"; b.type="button"; b.textContent="Sync Gmail"; b.onclick=()=>syncNow(true); host.appendChild(b); updateDashboardButton();
  }
