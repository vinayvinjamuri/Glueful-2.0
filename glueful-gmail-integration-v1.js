/* Glueful Gmail integration v8: multi-account UI without broad click interception. */
(function () {
  "use strict";
  ...
  async function loadStatus() { try { gmailStatus=await callGmail("status"); render(); updateDashboardButton(); configureAutoSync(); } catch(e) { console.warn("[Glueful] Gmail status:",e.message||e); } }
  ...
  function ensureDashboardButton() { ensureStyles(); if(!document.getElementById("glueful-dashboard-gmail-sync")){ const b=document.createElement("button"); b.id="glueful-dashboard-gmail-sync"; b.type="button"; b.textContent="Sync Gmail"; b.onclick=()=>syncNow(true); document.body.appendChild(b); } updateDashboardButton(); }
