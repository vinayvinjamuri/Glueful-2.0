/* Glueful Jobs logo recovery v3 — always attempts BrandFetch for rendered company cards. */
(function(){
  'use strict';
  const FLAG='__GLUEFUL_JOBS_LOGO_RECOVERY_V3__';
  ...
  function tryLogo(host,name){if(!host||host.dataset.logoV3==='loading'||host.dataset.logoV3==='ready')return;const d=domain(name);if(!d){fallback(host,name);return}host.dataset.logoV3='loading';const img=document.createElement('img');img.alt=name+' logo';img.loading='eager';img.decoding='async';img.referrerPolicy='no-referrer';const urls=[`https://cdn.brandfetch.io/${encodeURIComponent(d)}/w/128/h/128`,`https://www.google.com/s2/favicons?domain=${encodeURIComponent(d)}&sz=128`];let i=0;img.onerror=()=>{if(i<urls.length){img.src=urls[i++]}else{host.dataset.logoV3='fallback';fallback(host,name)}};img.onload=()=>{host.dataset.logoV3='ready'};host.replaceChildren(img);img.src=urls[i++]}
  function scan(){const r=root();if(!r)return;r.querySelectorAll('.g15-logo,.g15-company>div,.g15-row-logo,.g15-latest-logo').forEach(host=>{const card=host.closest('.g15-card,.g15-company,.g15-row,.g15-latest-card')||host.parentElement;const name=company(card||host);const img=host.querySelector('img');if(img){img.loading='eager';img.decoding='async';img.referrerPolicy='no-referrer';if(img.complete&&img.naturalWidth)host.dataset.logoV3='ready';else if(!img.dataset.logoV3Bound){img.dataset.logoV3Bound='1';img.addEventListener('error',()=>{host.dataset.logoV3='';tryLogo(host,name)},{once:true})}}else{tryLogo(host,name)}})}
  function boot(){scan();const r=root();if(r&&!r.dataset.logoV3Observer){r.dataset.logoV3Observer='1';new MutationObserver(()=>requestAnimationFrame(scan)).observe(r,{childList:true,subtree:true)}}[200,600,1200,2500,5000].forEach(t=>setTimeout(scan,t))}
