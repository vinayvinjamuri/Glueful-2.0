/* Glueful UI Stability V1 — fixes dashboard reflow/jank without changing feature logic. */
(function(){
  'use strict';
  const STYLE_ID='glueful-ui-stability-v1';
  function install(){
    if(document.getElementById(STYLE_ID)) return;
    const s=document.createElement('style');
    s.id=STYLE_ID;
    s.textContent=`
      /* Stop visual reflow effects from the premium layer while the existing app settles. */
      #view-dashboard .card,
  ...
  function apply(){
    ...
    if(!d) return;
    const active=d.classList.contains('active') || d.style.display==='block';
    if(active) d.style.transform='none';
  }
  function start(){
    apply();
