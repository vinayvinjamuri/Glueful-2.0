...
(function(){
  'use strict';
  if(window.__GLUEFUL_JOBS_RESUME_ACTION_V3__) return;
  ...
  function patchDescription(){
    document.querySelectorAll('.g15-detail .job-description,.g15-detail .job-detail-block p,[data-job-description]').forEach(node=>{
      const raw=node.textContent||'';
      if(/&(?:lt|gt|amp|quot|#39|nbsp);/i.test(raw)||/<(?:span|p|div|br)\b/i.test(raw)){node.textContent=readableDescription(raw);node.style.whiteSpace='pre-line';}
    });
  }
  ...
  function openResume(job){
    if(!job?.id)return;
    const id=String(job.id);window.gluefulResumeJobContext={id,title:title(job),company:company(job),location:clean(job.location||job.city||job.job_location||''),source:job};
    document.querySelector('.g15-layer')?.remove();document.body.style.removeProperty('overflow');
  ...
  function captureInlineEdit(ev){
    ...
    if(!target)return;
    const onclick=String(target.getAttribute('onclick')||'');const m=onclick.match(/openJobResumeEditor\(['"]([^'"]+)['"]\)/);if(!m)return;
    const id=m[1];const jobs=getJobs();const job=jobs.find(j=>String(j.id)===id)||jobs.find(j=>String(j.sourceJobId||'')===id)||null;if(!job)return;
