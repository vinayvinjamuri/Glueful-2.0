(function(){
  'use strict';
  if(window.__GLUEFUL_MOBILE_UPDATE_GUARD_V1__) return;
  window.__GLUEFUL_MOBILE_UPDATE_GUARD_V1__=true;
  ...
  async function hardRefresh(){
    ...
    }catch(e){console.warn('[Glueful update] SW unregister failed',e)}
    try{
      if('caches' in window){
        const keys=await caches.keys();
        await Promise.all(keys.map(k=>caches.delete(k)));
      }
    }catch(e){console.warn('[Glueful update] cache purge failed',e)}
    const u=new URL(location.href);
    u.searchParams.set('glueful_refresh',String(Date.now()));
    location.replace(u.href);
  }

  function bindUpdateButton(){
    if(document.documentElement.dataset.gluefulUpdateBound==='1') return;
    ...
    document.documentElement.dataset.gluefulUpdateBound='1';
  }

  function drawer(){
    return document.querySelector('#glueful-drawer') || document.querySelector('.glueful-drawer') || document.querySelector('[data-drawer]') || null;
  }
  ...
  function replaceBrand(d){
    ...
    while(n=walker.nextNode()){
      if(norm(n.nodeValue)==='glueful'){ n.nodeValue='Quick Actions'; return; }
    }
  }
  ...
  function openPlugins(){
    ...
    if(!m){
      ...
      document.head.appendChild(s);document.body.appendChild(m);
      m.addEventListener('click',e=>{if(e.target===m||e.target.closest('.gmp-close'))m.remove()});
      m.querySelector('[data-gmp-refresh]')?.addEventListener('click',()=>{try{window.gluefulJobsLogoRefresh?.()}catch(_){};try{window.gluefulJobsResumeActionV1?.refresh?.()}catch(_) {}});
    }
  }

  function ensurePlugin(d){
    if(!d) return;
    if([...d.querySelectorAll('button,a,[role="button"]')].some(x=>norm(x.textContent).includes('plug-ins'))) return;
    ...
    b.type='button';b.className='gmp-nav-item';b.innerHTML='<span class="gmp-nav-icon">✦</span><span><strong>Plug-ins</strong><small>Explore and manage integrations</small></span><i>New</i>';
    b.onclick=openPlugins;
    if(placement&&placement.parentNode) placement.parentNode.insertBefore(b,placement.nextSibling);
    else if(account&&account.parentNode) account.parentNode.insertBefore(b,account);
    else d.appendChild(b);
    const s=document.createElement('style');s.textContent='.gmp-nav-item{width:100%;display:flex;align-items:center;gap:12px;text-align:left;border:0;background:transparent;color:#f5f7ff;padding:11px 12px;border-radius:12px;margin:3px 0;cursor:pointer}.gmp-nav-item:hover{background:rgba(123,54,255,.12)}.gmp-nav-icon{width:38px;height:38px;border-radius:10px;display:grid;place-items:center;background:#171d27;color:#b89cff;font-size:17px}.gmp-nav-item strong,.gmp-nav-item small{display:block}.gmp-nav-item strong{font-size:13px}.gmp-nav-item small{font-size:10px;color:#8993a5;margin-top:3px}.gmp-nav-item i{margin-left:auto;font-style:normal;font-size:8px;font-weight:900;color:#fff;background:linear-gradient(135deg,#7b36ff,#3e75ff);padding:4px 7px;border-radius:999px}';document.head.appendChild(s);
  }

  function patch(){bindUpdateButton();const d=drawer();if(d){replaceBrand(d);ensurePlugin(d)}}
