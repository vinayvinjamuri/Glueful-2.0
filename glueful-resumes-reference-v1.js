...
(function(){
  ...
  function install(){
    ...
    s.textContent=`
      @media(min-width:1101px){
        ...
        body #view-resume .gf-resumes-toolbar{display:flex!important;align-items:center!important;gap:10px!important;width:100%!important;margin:0 0 14px!important;padding:0!important;}
      ...
      @media(max-width:1100px){body #view-resume .gf-resumes-add-btn{min-height:42px!important;border-radius:10px!important;}body #view-resume .gf-resumes-company-card{background:#fff!important;color:#111827!important;border-color:#e1e6ef!important;}}
  ...
  function looksDark(el){try{const bg=getComputedStyle(el).backgroundColor||'';const m=bg.match(/rgba?\(([^)]+)\)/i);if(!m)return false;const p=m[1].split(',').map(x=>parseFloat(x.trim()));return p.length>=3&&p[0]<55&&p[1]<65&&p[2]<80;}catch(_){return false;}}
  function decorate(){
    const v=document.getElementById(VIEW_ID);if(!v||!isResumeView())return;install();
    Array.from(v.children).forEach(el=>{if(el.matches('.view-header,.gf-resumes-toolbar,.gf-resumes-info,.gf-resumes-add-btn'))return;if(looksDark(el)||/resume|company|group/i.test(String(el.className||''))){if(el.getBoundingClientRect().height>45)el.classList.add('gf-resumes-company-card');}});
    ...
    if(inputs.length){let toolbar=inputs[0].closest('.toolbar,.search-bar,.filter-bar,[class*="toolbar"],[class*="search"]');if(!toolbar)toolbar=inputs[0].parentElement;if(toolbar&&!toolbar.classList.contains('view-header'))toolbar.classList.add('gf-resumes-toolbar');if(toolbar&&!inputs[0].parentElement.classList.contains('gf-resumes-search-wrap'))inputs[0].parentElement.classList.add('gf-resumes-search-wrap');filterButtons.forEach(b=>b.classList.add('gf-resumes-filter-btn'));}
    addHeaderAction(v);
  }
  function addHeaderAction(v){
    if(v.querySelector('.gf-resumes-add-btn'))return;const header=v.querySelector('.view-header');if(!header)return;
  ...
  function start(){install();decorate();[150,500,1000,1800].forEach(ms=>setTimeout(decorate,ms));document.addEventListener('click',()=>setTimeout(decorate,80),true);}
